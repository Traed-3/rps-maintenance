...
export default async function NewInvoicePage({ searchParams }: { searchParams: Promise<{ job?: string; department?: string }> }) {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      <Rev19Builder action={saveInvoice.bind(null, null)} header={{ kind: 'invoice', job_id: job ?? null, department: department ?? 'construction' } as never} customers={lists.customers} jobs={lists.jobs} rateCards={lists.rateCards} />
    </div>
  )
}
