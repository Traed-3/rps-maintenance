...
export default async function TiresMaintenancePage() {
  ...
  const { data: assets } = await admin
    .from('assets')
    .select('id, unit_number, name, make, model, year, status, last_tire_service_date, last_tire_service_mileage, next_tire_inspection_date')
    .eq('company_id', profile!.company_id)
    .neq('status', 'retired')
    .order('unit_number')

  const rows = (assets ?? []).map((a) => {
    const due = calcDateDue(a.next_tire_inspection_date)
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      <div className="mb-6 flex items-center gap-3">
        ...
        <h1 className="inline-flex items-center gap-2.5 text-2xl sm:text-3xl font-bold text-gray-900 tracking-tight before:content-[''] before:w-1.5 before:h-7 before:rounded-full before:bg-gradient-to-b before:from-blue-500 before:to-blue-700 before:shrink-0">Tires</h1>
      </div>
              ...
              {sorted.map((a) => (
                <ClickableRow key={a.id} href={`/assets/${a.id}`}>
                  ...
                  <td className="px-4 py-3 text-gray-500 hidden md:table-cell">
                    {a.last_tire_service_date
                      ? new Date(a.last_tire_service_date).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })
                      : '—'}
                  </td>
                  <td className="px-4 py-3 text-right">
                    <Link href={`/assets/${a.id}/maintenance/tires`} className="text-xs font-medium text-blue-600 hover:text-blue-800">Record →</Link>
                  </td>
                </ClickableRow>
              ))}
            </tbody>
