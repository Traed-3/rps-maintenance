...
export default async function MileagePage({
  ...
  const { data: asset } = await admin
    .from('assets')
    .select('id, unit_number, name, make, model, year, current_mileage, next_oil_change_mileage')
    .eq('id', id)
  ...
  return (
    <div className="p-6 max-w-lg mx-auto">
      <div className="mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 mt-2">Add Mileage</h1>
        {vehicleLabel && <p className="text-gray-500 text-sm mt-0.5">{vehicleLabel}</p>}
      </div>

      {/* Current mileage callout */}
      <div className="bg-blue-50 border border-blue-100 rounded-xl px-5 py-4 mb-6">
        ...
        {asset.next_oil_change_mileage != null && (
      ...
      <div className="bg-white rounded-xl border border-gray-200 p-6">
        <MileageForm action={action} today={today} />
      </div>
    </div>
  )
}
